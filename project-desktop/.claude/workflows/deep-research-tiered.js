export const meta = {
  ...
  description: 'Deep research with Sonnet worker agents (search/fetch/verify) and orchestrator-model scope + synthesis.',
  whenToUse: 'When the user wants a deep, multi-source, fact-checked research report and wants the investigation fan-out to run on a cheaper/faster worker model (default sonnet) while the current session model handles decomposition and final synthesis. Pass the question as args (string), or {question, workerModel} to override the worker tier.',
  phases: [{"title":"Scope","detail":"Decompose question into 5 search angles (orchestrator model)"},{"title":"Search","detail":"5 parallel WebSearch agents, one per angle (worker model)"},{"title":"Fetch","detail":"URL-dedup, fetch top 15 sources, extract falsifiable claims (worker model)"},{"title":"Verify","detail":"3-vote adversarial verification per claim (worker model)"},{"title":"Synthesize","detail":"Merge dupes, rank by confidence, cite sources (orchestrator model)"}],
}

// deep-research-tiered: same architecture as built-in deep-research, but
// search/fetch/verify workers run on WORKER_MODEL (default 'sonnet') while
// Scope and Synthesize omit `model` and inherit the session/orchestrator model.
// args: '<question>'  OR  { question: '<question>', workerModel: 'sonnet'|'haiku'|'opus' }
...
// ─── Args: string or {question, workerModel} ───
...
// ─── Phase 0: Scope — decompose question (orchestrator model: no `model` opt) ───
phase("Scope")
if (!QUESTION) {
  return { error: "No research question provided. Pass it as args: Workflow({name: 'deep-research-tiered', args: '<question>'}) or args: {question, workerModel}." }
}
log("Worker model: " + WORKER_MODEL + " · Scope/Synthesize: session model")
...
const searchResults = await pipeline(
  ...
  angle => agent(SEARCH_PROMPT(angle), {
    label: "search:" + angle.label, phase: "Search", schema: SEARCH_SCHEMA, model: WORKER_MODEL
  }).then(r => {
    if (!r) return null
...
// ─── Verify: 3-vote adversarial — worker model ───
phase("Verify")
...
// ─── Synthesize — orchestrator model (no `model` opt: inherits session model) ───
phase("Synthesize")
